...
const refreshAccessToken = async (
  ...
  try {
    if (isServer) {
      if (!cookieString) throw new Error('No cookies provided for server-side refresh');
      console.log('Server-side refresh with cookies:', cookieString);
      const response = await clientApi.post('/api/auth/refresh', {}, {
  ...
  api.interceptors.response.use(
    ...
    async (error: AxiosError) => {
      ...
      if (error.response?.status === 401 && !originalRequest._retry) {
        ...
        isRefreshing = true;
        try {
          const cookieString = isServer ? (originalRequest.headers?.['Cookie'] as string) : undefined;
          const refreshResult = await refreshAccessToken(isServer, cookieString);
          ...
          if (isServer && refreshResult.newCookies) {
            console.log('Updating request cookies with:', refreshResult.newCookies);
